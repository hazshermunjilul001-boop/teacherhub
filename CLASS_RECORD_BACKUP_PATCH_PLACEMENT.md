...
The backup button now downloads a machine-readable JSON Restore Backup first, followed by the Excel file generated by the E-Class Record download button for regular subjects. GMRC/Values Print Preview now has a working Download Excel button, and its backup action reuses that same preview Excel export. The PDF download control was removed from the GMRC/Values preview because the browser Print action is the working PDF/print route. JSON and Excel files can both be uploaded through the restore button on the matching regular-subject or GMRC/Values class-record page.
