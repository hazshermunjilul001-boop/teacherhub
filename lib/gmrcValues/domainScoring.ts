...
export function domainSummary(scores: DomainScores | null | undefined) {
  const source = scores || {};
  const summaries = GMRC_VALUES_BLOCKS.map(block => {
    ...
    const highest = block.count * 100;
    const ps = block.kind === 'examination'
      ? (values[0] * 0.30 + values[1] * 0.30 + values[2] * 0.40)
      : (highest > 0 ? (total / highest) * 100 : 0);
    const ws = ps * block.weight;
    return { ...block, values, total, entered, highest, ps, ws, average: entered ? total / entered : 0 };
  });
...
export function gmrcValuesTermGrade(scores: DomainScores | null | undefined, transmute: (value: number) => number) {
  const summary = domainSummary(scores);
